...
if (!supabaseUrl || !supabaseKey) {
  console.error("Env değişkenleri eksik. .env.local dosyasını kontrol edin.");
  process.exit(1);
}
...
const indicators = [
  ...
  },
  {
    ...
    platform: "TradingView",
    short_desc: "BIST hisselerinde OBO, üçgen, kanal, ikili tepe/dip ve Fibonacci formasyonlarını otomatik tespit eden, gerçek zamanlı kırılım taraması yapan kapsamlı Pine Script v5 indikatörü.",
    description: `RdAlgo Otomatik Formasyon ve Tarama İndikatörü, BIST hisselerinde teknik analizin temel taşlarını oluşturan formasyon yapılarını tamamen otomatik olarak tespit eden ve tarayan güçlü bir Pine Script v5 aracıdır.\n\n**Tespit Edilen Formasyonlar:**\n- Omuz-Baş-Omuz (OBO) ve Ters OBO\n- Yükselen, Alçalan ve Simetrik Üçgen\n- İkili Tepe (M) ve İkili Dip (W)\n- Fibonacci geri çekilme bölgeleri`,
    images: [],
  ...
  },
  {
    ...
    description: `RD Trend Formasyon V1, fiyat hareketinin yapısını okuyarak kısa ve uzun vadeli trend kanallarını otomatik olarak grafik üzerine çizen bir Pine Script indikatörüdür.\n\n**Öne Çıkan Özellikler:**\n- Kısa vadeli trend için beyaz alt/üst trend çizgileri\n- Uzun vadeli trend yapısı için renkli dinamik kanallar\n- Trend kırılımlarını otomatik işaretleme`,
    images: [],
  ...
  },
  {
    ...
    description: `RDTREND, işlemlerdeki hata marjını en aza indirmek ve ana trend yönünü net biçimde ortaya koymak amacıyla geliştirilmiş kapsamlı bir Pine Script trend indikatörüdür.\n\n**Öne Çıkan Özellikler:**\n- Ana trend yönünü tek bakışta gösteren renk kodlaması\n- Trend güç ve zayıflık bölgelerinin görsel gösterimi\n- Her zaman dilimine uyarlanabilir yapı`,
    images: [],
...
async function seed() {
  console.log(`${indicators.length} indikatör Supabase'e aktarılıyor...`);
  ...
  for (const ind of indicators) {
    ...
    if (error) {
      console.error(`❌ HATA (${ind.slug}):`, error.message);
    } else {
