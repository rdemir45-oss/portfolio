...
export default function Announcements() {
  ...
  const categories: (PostCategory | "Tümü")[] = [
    "Tümü",
    "Teknik Analiz",
    "Eğitim",
    "Duyuru",
          ...
          {filtered.map((post, i) => {
            ...
            return (
              ...
              >
                ...
                >
                  {/* Üst: kategori + pin + tarih */}
                  <div className="flex items-center justify-between mb-4">
                    <span
                      className={`flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full border ${cfg.bg} ${cfg.border} ${cfg.color}`}
                    >
                      {cfg.icon}
                      {post.category}
                    </span>
                    <div className="flex items-center gap-2">
                      {post.pinned && (
                        <TbPin size={15} className="text-emerald-500 rotate-45" />
                      )}
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {post.tags.slice(0, 4).map((tag) => (
                      <span
                        key={tag}
                        className="text-xs text-slate-500 bg-slate-800/60 border border-slate-700/50 px-2 py-0.5 rounded"
                      >
                        {tag}
                      </span>
                    ))}
